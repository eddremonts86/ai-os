...
## 0. Brand resolution (AI-OS)

**The brand is already onboarded. Do not run the onboarding flow, and do not ask the user
about tokens.**

This skill is installed as a symlink into the AI-OS repo and shared by every project and all
six CLIs. Upstream's first-run gate assumed a per-project copy and wrote the answer into
`references/style-guide.md` — here that write changes the skin globally and dirties the AI-OS
repo. See [`VENDORED_FROM.md`](VENDORED_FROM.md).

Resolve tokens in this order, first hit wins:

1. **`.ai-os/brand-tokens.md`** in the current project root (or the nearest ancestor). Present
   only for work under a different brand — client projects. Use it verbatim and ignore steps 2–3.
2. **[`references/style-guide.md`](references/style-guide.md)** — the AI-OS skin, dark-first.
   This is the normal path. Default variant is `template-dark.html`, not `template.html`.
3. If neither resolves, stop and say so. Never fall back to upstream's white-smoke +
   atomic-tangerine default — shipping a generic-skinned diagram into a branded repo is the
   exact failure this section exists to prevent.

Canonical brand source is `context/06_brand.md` in the AI-OS repo. To change the brand, edit
that file and propagate to `references/style-guide.md` in the same commit — never edit only
the style guide.

[`references/onboarding.md`](references/onboarding.md) stays in the skill for one purpose:
building a `.ai-os/brand-tokens.md` for a new client brand. It must write to that file, never
back into `style-guide.md`.
...
- The acid accent is **editorial, not a flag.** 1–2 focal nodes per diagram. Using it on 5 nodes erases the signal.
...
| Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in Space Grotesk. |
...
| Accent on every "important" node | The acid accent is 1–2 editorial focals, not a signaling system |
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). The active skin is the **AI-OS brand** (dark-first: `void` paper, `ink-white` text, `acid` accent, `cyan` links, blue-violet neutrals), sourced from `context/06_brand.md`. See §0 for brand resolution — do not run onboarding against `style-guide.md`.
...
**Mono is for technical content.** Names are Space Grotesk. Page title is Instrument Serif. Italic Instrument Serif is reserved for annotation callouts. Never use mono as a blanket "dev" font — scope it to technical content.
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="1" cy="1" r="0.9" fill="rgba(244,245,249,0.10)"/>
  </pattern>
</defs>
<rect width="100%" height="100%" fill="#08090e"/>
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#a2a4ae"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#b3ee55"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#3bdfef"/>
</marker>
```

| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#a2a4ae` | Internal, generic |
| Accent | acid `#b3ee55` | Primary / highlighted / headline |
| Link-blue | `#3bdfef` | HTTP/API calls, external systems |
...
<rect x="X" y="Y" width="W" height="H" rx="6" fill="#08090e"/>
<!-- 2. Styled box -->
...
<text x="X+22" y="Y+15" fill="STROKE@0.8" font-size="7" font-family="'JetBrains Mono', ui-monospace, monospace"
      text-anchor="middle" letter-spacing="0.08em">API</text>
<!-- 4. Node name (Space Grotesk — human-readable) -->
<text x="CX" y="CY+2" fill="#f4f5f9" font-size="12" font-weight="600"
      font-family="'Space Grotesk', system-ui, sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (JetBrains Mono) -->
<text x="CX" y="CY+18" fill="#a2a4ae" font-size="9"
      font-family="'JetBrains Mono', ui-monospace, monospace" text-anchor="middle">tech:port</text>
```
...
<rect x="MID_X-18" y="ARROW_Y-20" width="36" height="12" rx="2" fill="#08090e"/>
<text x="MID_X" y="ARROW_Y-11" fill="#7d8089" font-size="8"
      font-family="'JetBrains Mono', ui-monospace, monospace" text-anchor="middle" letter-spacing="0.06em">WRITE</text>
```
...
<line x1="30" y1="LEGEND_Y-8" x2="VIEWBOX_W-30" y2="LEGEND_Y-8"
      stroke="rgba(244,245,249,0.10)" stroke-width="0.8"/>
<text x="30" y="LEGEND_Y+8" fill="#a2a4ae" font-size="8" font-family="'JetBrains Mono', ui-monospace, monospace"
...
1. **Header** — eyebrow (JetBrains Mono), title (Instrument Serif), optional subtitle (Space Grotesk muted).
...
- `background: paper-2` (not paper — slight lift without shadow)
...
- [ ] Acid accent used on ≤2 elements? If more, which actually deserve focal status?
...
- [ ] Every arrow label has an opaque `fill="#08090e"` rect behind it?
...
- [ ] Human-readable names in Space Grotesk, not JetBrains Mono?
...
- [ ] Mono scoped to technical content only — never on human-readable names?
...
|---|---|---|
| **Minimal dark** (default in AI-OS) | `template-dark.html`, `example-<type>-dark.html` | Default. Matches the dark-first AI-OS brand: site, docs, slides, social. |
| **Minimal light** | `template.html`, `example-<type>.html` | Print, PDF, or a light-background document. Uses the light register in `style-guide.md`. |
