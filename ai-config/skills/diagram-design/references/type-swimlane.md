...
## Layout conventions
- Horizontal lanes (or vertical columns) — one per actor/team. Label each lane in the left margin (or top) with a JetBrains Mono eyebrow.
...
- Handoffs (arrows crossing lane boundaries) are the most important edges — consider acid accent on the handoff that introduces the most coupling or latency.
