...
**Goal:** point the skill at a design source — a website, an installed skill, or a local
folder — and have it extract the palette + typography, then write the result as a set of
semantic-role tokens.

Takes about 60 seconds.

> **AI-OS: write target is NOT `style-guide.md`.**
>
> Upstream rewrites `style-guide.md` in place. Here that file holds the AI-OS brand and is
> shared by every project and all six CLIs via symlink — rewriting it re-skins every project
> and dirties the AI-OS repo. See [`../SKILL.md` §0](../SKILL.md).
>
> Run this flow **only** to onboard a *different* brand for one project (client work), and
> write the output to **`.ai-os/brand-tokens.md` in that project's root**, using the same
> semantic role names as `style-guide.md`. Everywhere below that says "rewrite
> `style-guide.md`", read it as "write `.ai-os/brand-tokens.md`".
>
> To change the AI-OS brand itself, edit `context/06_brand.md` in the AI-OS repo and propagate
> to `style-guide.md` in the same commit — do not run this flow.
...
If the site has only one family, keep the AI-OS defaults for the missing roles (Instrument Serif for title, JetBrains Mono for mono). Don't force-pick a mono font that isn't on the site.
