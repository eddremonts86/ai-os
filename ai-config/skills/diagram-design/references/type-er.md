...
- Each entity is a two-section box:
  ...
  - **Body**: field list in JetBrains Mono, one per line. PK prefixed with `#`, FK prefixed with `→`.
- Relationships: lines between entities with cardinality at each end:
  - `1`, `N`, `0..1`, `1..*` in JetBrains Mono, 8px, placed 10–12px from the entity edge.
...
- Acid accent on the aggregate root or central entity of the model.
