...
- Circle fill: very low-opacity tint — `rgba(244,245,249,0.04)` for ink set, `rgba(79,93,117,0.05)` for muted. Tints compound naturally in overlap regions.
...
- **Set labels** placed outside the circle, NEVER crossing the stroke. Geist 12–14px 600 for the set name, optional JetBrains Mono 9px sublabel.
...
- **Acid accent** on the ONE focal intersection — the "sweet spot". Either acid accent label stroke OR clipPath-bounded acid accent fill tint (`rgba(179,238,85,0.10)`).
...
- Acid accent on multiple overlap regions (focal signal dies).
