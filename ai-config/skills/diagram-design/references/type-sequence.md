...
- Return messages: **dashed** stroke + **filled** marker (never open). Prefer muted; optionally match the originating call color when pairing multi-hop stacks. Headline success may use solid acid (see Message kinds).
- Acid accent on the primary success response or headline message — one, maybe two. Actor focal strokes do not count toward the acid accent message budget.
...
<marker id="arrow-open" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polyline points="0 0, 8 3, 0 6" fill="none" stroke="#a2a4ae" stroke-width="1.2"/>
</marker>
```

Dark mode: stroke `#2b2d38` (muted on dark paper). Do not fill the open marker — the hollow head is the async signal. Return messages keep the **filled** marker even when dashed.

## Combined fragments (`alt` / `opt` / `loop`)

Use a rectangular **frame** that spans only the lifelines participating in the branch. Operator label is JetBrains Mono, uppercase, in a small tab at the top-left of the frame. Time still flows top→down inside the frame.
...
<rect x="X" y="Y" width="W" height="H" rx="4"
      fill="rgba(244,245,249,0.02)" stroke="rgba(244,245,249,0.22)" stroke-width="1"/>
<!-- Operator tab -->
<rect x="X" y="Y" width="40" height="16" rx="2"
      fill="#08090e" stroke="rgba(244,245,249,0.22)" stroke-width="1"/>
<text x="X+20" y="Y+12" fill="#a2a4ae" font-size="8"
      font-family="'JetBrains Mono', ui-monospace, monospace" text-anchor="middle"
      letter-spacing="0.12em">ALT</text>
```

Dark mode: frame fill `rgba(8,9,14,0.04)`, stroke `rgba(8,9,14,0.22)`, tab fill = dark `paper` (`#f4f5f9`), tab text = dark `muted` (`#2b2d38`).

### Operators

| Operator | Regions | Divider | Guard label |
|---|---|---|---|
| `opt` | 1 | none | `[if condition]` under the tab (JetBrains Mono 8px) |
...
<text x="X+12" y="GUARD_Y" fill="#a2a4ae" font-size="8"
      font-family="'JetBrains Mono', ui-monospace, monospace" letter-spacing="0.04em">[token valid]</text>

<!-- alt region divider -->
<line x1="X+8" y1="DIV_Y" x2="X+W-8" y2="DIV_Y"
      stroke="rgba(244,245,249,0.20)" stroke-width="1" stroke-dasharray="4,3"/>
```
...
- Acid accent stays on **one** headline success message across the whole diagram (usually the happy-path return inside the first `alt` region, or the final success outside a loop). Do not acid accent both `alt` branches.
...
- Max acid accent elements: 2 (prefer 1 for fragment diagrams).
...
<line x1="CX" y1="TOP" x2="CX" y2="BOTTOM"
      stroke="rgba(244,245,249,0.20)" stroke-width="1" stroke-dasharray="3,3"/>
```

## Activation bar primitive
```svg
<rect x="CX-4" y="TOP" width="8" height="H"
      fill="rgba(244,245,249,0.06)" stroke="#a2a4ae" stroke-width="0.8"/>
```
...
- Fragment operator label in Space Grotesk — must be mono: `ALT` / `OPT` / `LOOP`.
- Acid accent on both `alt` branches.
