...
## Layout conventions
- Root owner or front door at top center. Use one acid focal node for the person/team/agent that receives ambiguous work.
...
- Each node should answer three questions when space allows:
  1. **Name** — human-readable role/person/agent in Space Grotesk.
  2. **How to invoke** — Slack handle, queue, issue prefix, or trigger in JetBrains Mono.
...
- Max acid accent nodes: 1. The org chart's job is clarity, not highlighting everything.
- Max side callouts: 2.
