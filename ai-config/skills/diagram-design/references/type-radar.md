...
- **Axis labels:** one word per spoke (Jobs-minimal). Space Grotesk 11px weight 600. Place 16px outside the outer ring along the axis vector. Top/bottom = `text-anchor="middle"`; right side = `start`; left side = `end`.
- **Scale ticks** (e.g. `2 4 6 8 10`) only on the **first (top) axis** — putting numbers on every spoke clutters the chart fast. JetBrains Mono 8px, `muted`, anchored end at `cx − 6`.
...
- **Legend:** horizontal strip at the bottom (per the global rule). Swatch is a 16×8 rectangle (matches the polygon stroke+fill, not a circle), then the entity name. ~140px between entries. Optional italic tail on the right with the rationale (`"One acid accent. Position is the signal — color reserved for the recommended option."`).
...
<polygon points="500,96 622,201 585,356 415,356 363,196"
         fill="rgba(179,238,85,0.18)" stroke="#b3ee55" stroke-width="1.8"/>
```
...
- **Mono-font axis labels.** Names go in Space Grotesk (the global rule). Mono is for technical sublabels only.
