...
- **Axes:** X at y=420 (baseline), Y at x=80. Both use JetBrains Mono 8px gridline labels. Gridlines 4–6 per axis at equal intervals.
...
- **Labels on points (optional):** JetBrains Mono 8px next to a point. Use a paper-fill rect mask behind the label. Label at most 2–3 points; not all.
- **Trend line (optional):** `<line>` from lower-left to upper-right, stroke `rgba(244,245,249,0.25)` dashed 4,3. Never force a perfect fit — only add if the trend is visually obvious.
- **Quadrant dividers (optional):** light dashed lines at the median x and y to split into quadrants. Label each quadrant in JetBrains Mono 8px, muted.

### Point pattern

```svg
<!-- Non-focal point — paper mask + circle -->
<circle cx="X" cy="Y" r="5" fill="#08090e"/>
<circle cx="X" cy="Y" r="5" fill="rgba(79,93,117,0.20)" stroke="#a2a4ae" stroke-width="1"/>

<!-- Focal point -->
<circle cx="X" cy="Y" r="6" fill="#08090e"/>
<circle cx="X" cy="Y" r="6" fill="rgba(179,238,85,0.15)" stroke="#b3ee55" stroke-width="1.2"/>
```
