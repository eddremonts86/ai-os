...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill rgba(244,245,249,0.10)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `rgba(244,245,249,0.018)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `rgba(244,245,249,0.12)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `rgba(244,245,249,0.20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `rgba(244,245,249,0.12)`, number text ink.
- Focal fill: `rgba(179,238,85,0.20)`, number text accent (§5).
...
- Font: JetBrains Mono 6 px, weight 500, `letter-spacing="0.12em"`, `text-anchor="middle"`.
- Default fill: muted (`#a2a4ae` light / `#2b2d38` dark).
- Focal fill: accent (`#b3ee55` light / `#b3ee55` dark).
...
|---|---|---|---|---|---|
| `normal` | `#a2a4ae` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `#b3ee55` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `#a2a4ae` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="rgba(244,245,249,0.10)"/>
  </pattern>
...
| Lane tint | `rgba(244,245,249,0.018)` | `rgba(8,9,14,0.025)` |
| Dividers | `rgba(244,245,249,0.12)` | `rgba(8,9,14,0.12)` |
| Label col divider | `rgba(244,245,249,0.20)` | `rgba(8,9,14,0.22)` |
| Default chip fill | `rgba(244,245,249,0.12)` | `rgba(8,9,14,0.12)` |
| Focal chip fill | `rgba(179,238,85,0.20)` | `rgba(179,238,85,0.22)` |
| Default node fill | white | `rgba(8,9,14,0.04)` |
| Default node stroke | `rgba(244,245,249,0.25)` | `rgba(8,9,14,0.20)` |
| Focal node fill | `rgba(179,238,85,0.08)` | `rgba(179,238,85,0.12)` |
| Focal node stroke | `#b3ee55` | `#b3ee55` |
