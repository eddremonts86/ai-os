...
- **Left label column:** x=20–200 (180px). Task names in Space Grotesk 11px 600. Phase labels as JetBrains Mono 7px eyebrows above each group.
...
- **Time axis:** JetBrains Mono 8px week/month labels at x=200+i×pitch, y=56 (just above first task row). A hairline separator at y=64.
- **Phase grouping:** a subtle zone rect (same pattern as architecture zone) behind each phase's rows, with an eyebrow label in the top-left margin. Use `rgba(244,245,249,0.02)` fill, `rgba(244,245,249,0.10)` stroke.
...
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(79,93,117,0.15)" stroke="#a2a4ae" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#f4f5f9" font-size="10" font-weight="600"
      font-family="'Space Grotesk', system-ui, sans-serif">Task name</text>

<!-- Focal task -->
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(179,238,85,0.12)" stroke="#b3ee55" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#b3ee55" font-size="10" font-weight="600"
      font-family="'Space Grotesk', system-ui, sans-serif">Key task</text>
```
