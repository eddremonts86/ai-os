...
focal_accent: "#b3ee55"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(244,245,249,0.05)`, stroke `rgba(244,245,249,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(79,93,117,0.06)`, stroke `#7d8089`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#b3ee55` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#a2a4ae` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#3bdfef` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#a2a4ae` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#b3ee55` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |

**Defs block** (required, five markers — exactly):

```svg
<defs>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#a2a4ae"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#b3ee55"/></marker>
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#3bdfef"/></marker>
  <marker id="arrow-sm"     markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto"><polygon points="0 0, 6 2.5, 0 5" fill="#a2a4ae"/></marker>
  <marker id="arrow-dim"    markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="rgba(244,245,249,0.45)"/></marker>
</defs>
```
...
- **Labels:** every `primary`, `secondary`, `federated`, `auth` edge gets a protocol label (JetBrains Mono 8 px, paper-filled rect mask with 6–10 px clear gap above the stroke). `trigger` edges are unlabelled.
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#08090e` | `#f4f5f9` |
| Ink | `#f4f5f9` | `#08090e` |
| Muted | `#a2a4ae` | `#2b2d38` |
| Accent | `#b3ee55` | `#b3ee55` |
| Link (federated) | `#3bdfef` | `#3bdfef` |
| Side-column fill | `rgba(79,93,117,0.06)` | `rgba(8,9,14,0.06)` |
| Side-column stroke | `#7d8089` | `rgba(8,9,14,0.30)` |
| Zone fill | `rgba(244,245,249,0.025)` | `rgba(8,9,14,0.04)` |
| Zone stroke | `rgba(244,245,249,0.32)` | `rgba(8,9,14,0.30)` |
| Non-focal bar fill | `rgba(244,245,249,0.05)` | `rgba(8,9,14,0.06)` |
| Focal fill | `rgba(179,238,85,0.08)` | `rgba(179,238,85,0.12)` |
| Focal stroke | `#b3ee55` | `#b3ee55` |
...
- **Per-tool color coding** (teal-NiFi, magenta-MinIO, yellow-Jupyter) inside the zone — collapses hierarchy; only the two focal accents earn acid accent, plus up to 2 custom colors on cross-cutting components (§4 cap).
