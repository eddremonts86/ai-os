...
## Labeled Variants (A / B / C)

When producing multiple distinct options (mocks, layouts, copy directions), label each
one visibly — `A`, `B`, `C`… — and lay them out for direct side-by-side comparison in a
single file. This lets the user answer with a terse composite like "B plus D" or "C but
with A's hero" instead of describing the option in prose. Keep one file across iterations
so the reference URL stays stable, and never renumber options between rounds — if an
option dies, keep its label retired rather than shifting the others.
