...
_Based on source brief:_ Cursor stores chat session history in SQLite but lacks a way to delete individual chat sessions or by folder or repository.After accumulating hundreds of sessions, I needed a way to clean up specific conversations and even chat sessions from specific folders.With that in mind I built a small Python CLI that let you do that.It can be installed with brew: brew install vilaca/tap/cursor-chat-cleaner
