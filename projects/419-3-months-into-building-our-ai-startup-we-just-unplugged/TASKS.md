...
- [ ] Read the source at `https://www.reddit.com/r/startups/comments/1vn0qa9/3_months_into_building_our_ai_startup_we_just/` and quote the original problem statement into a `source-quote.md` for traceability.
- [ ] Create the project folder under `apps/419-3-months-into-building-our-ai-startup-we/` (or per the chosen deployment target).
...
- [ ] Implement the data model: `signals`, `costs`, `checklists`, `sketches`, `responses`
- [ ] Build the frame UI: one page per signal, with the named threshold
- [ ] Wire the OpenAI API for the rebuild sketch draft per response
- [ ] Add structured logging on every frame query so failures are reproducible
...
- [ ] Run a single end-to-end smoke test of the 419-3-months-into-building-our-ai-start MVP against the source post's stated use case.
