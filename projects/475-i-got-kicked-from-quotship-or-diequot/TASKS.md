...
- [ ] Read the source at `https://www.reddit.com/r/indiehackers/comments/1vi0q98/i_got_kicked_from_ship_or_die/` and quote the original problem statement into a `source-quote.md` for traceability.
- [ ] Create the project folder under `apps/475-i-got-kicked-from-ship-or-die/` (or per the chosen deployment target).
...
- [ ] Public demo day
- [ ] No-penalty reminders

## Phase 2: Deploy

- [ ] Confirm the chosen deployment target for the stack (Next.js, TypeScript, PostgreSQL) and set the prod-equivalent env vars.
- [ ] Run a single end-to-end smoke test of the 475-i-got-kicked-from-ship-or-die MVP against the source post's stated use case.
