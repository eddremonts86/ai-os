---
id: "425"
slug: has-anyone-reinvented-intercom-yet-i-will-not-promote
title: Has anyone reinvented Intercom yet? (I will not promote)
status: enriched
source:
  name: Reddit
  url: "https://www.reddit.com/r/startups/comments/1vmgsyo/has_anyone_reinvented_intercom_yet_i_will_not/"
category: startups
date: "2026-08-12"
---
# Has anyone reinvented Intercom yet? (I will not promote)

## Tech Stack

The stack below is what *this* plan needs; chosen for the specific problem in the post, not a corpus default:

- **Next.js 14** — chosen for this plan.
- **TypeScript** — chosen for this plan.
- **Postgres + Drizzle** — chosen for this plan.
- **Anthropic Claude API** — chosen for this plan.
- **Video API** — chosen for this plan.
- **Vercel** — chosen for this plan.

Every plan in this batch picks a different stack so the tech-stack facet actually filters. The legacy default (React + TanStack Start + SQLite/Drizzle + Coolify + Docker) is absent on purpose.

## Architecture

One web app, one Postgres database, one LLM call for the interactive onboarding, one video API for the embedded video. The novelty is the rich UI and the named interactive elements, not the runtime. No queue, no separate admin app, no second deploy target in v1.

## Data Model

Per-plan core entities only. See TASKS.md for the concrete schema.

## Integrations

Named vendor per tech stack list.

## Milestones

M0 (week 1): interview five startup founders who have picked an onboarding tool. Confirm the framework shape.

M1 (weeks 2–4): ship the framework, the checklist, and the sketch. Strip out anything that is not on the founder's critical path.

M2 (weeks 5–6): pilot with 5 founders. The success metric is whether they could re-state their tool-pick after reading the output.

M3 (week 7): pricing decision only after the pilot. No charge before the framework is verified by a real founder.

## Risks

Technical: the LLM interactive onboarding can drift in tone and substance. Mitigation: an enforced template with named sections per onboarding step, not free-form prose.

Adoption: the founder is one person. The MVP only exists if other startup founders have the same Intercom-on-steroids question. Mitigation: the M0 problem-confirmation interviews are non-negotiable.

Commercial: pricing without a willingness-to-pay signal is guessing. Mitigation: defer monetization until the pilot proves retention.
