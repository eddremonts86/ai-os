...
The reference documentation grid (`ConfigReference` with its 72 collapsible `ConfigField` cards) **must remain on the page** as a separate section after the playground. It is the explanatory complement: the playground lets you *touch* the config, the doc grid explains *what each field does* with copy-able examples. The two are siblings, not redundant — the playground collapses to "control widget" UI, the doc grid keeps full prose + examples per field.
...
- [ ] The **ConfigReference documentation grid** (the 72-card section that explains every field with description, notes, and copy-able example) renders as a separate section after the playground under a "Config reference" / `#config-reference` anchor, and the `SiteNav` shows a "Config" link to it. The grid must re-appear (it was removed in Task 1 — see "Non-goals" caveat below).
- [ ] Form field labels are **human-readable, Title Case, and contextual**. The raw config key (camelCase, dotted paths like `credentials.apiKey`) must NOT be used as the visible label. Each label answers "what does this control?" in a way that disambiguates between sibling fields with the same key (`provider.kind` vs `model.provider`, etc.). Labels use the `name` field from `CONFIG_FIELDS` in `demo/src/content/config-reference.ts`, which is already a curated, prose-friendly label per field.
- [ ] `pnpm typecheck` and `pnpm build` both pass with zero new errors. `pnpm test:unit` passes (existing tests untouched).
- [ ] Browser smoke test: open `http://127.0.0.1:5173/#live-demo`, confirm only one `<ChatPanel />` is on the page, mutate 5+ different fields across at least 3 sections (provider, model, UI), confirm each mutation visibly affects the panel state without remounting. Then scroll to `#config-reference` and confirm the doc grid renders with all 72 cards. Screenshot captured for the report.
...
- [ ] Update `demo/src/components/ConfigReferenceSection.tsx` to no longer render `<LiveConfigPlayground />` — it only renders `<ConfigReference />` (the doc grid). **The section file itself is kept** so we can re-mount `<ConfigReferenceSection />` from `App.tsx` in block 2.
...
- [ ] **Re-mount `<ConfigReferenceSection />`** in `App.tsx` after `<LiveDemoSection />`. The doc grid re-appears as a separate section under `#config-reference`. The `SiteNav` already has a "Config" link to it.
- [ ] **Verify:** `pnpm typecheck && pnpm build` pass; browser shows the Coding Buddy panel on the left and an empty card on the right at `lg` viewport; `ChatHeader` shows "Coding buddy" persona; form side has no content yet but layout is solid; scrolling to `#config-reference` reveals the doc grid with 72 cards.
